...
const VideoCaptionGenerator: React.FC<Props> = ({ options }) => {
  ...
  const handleGenerateCaption = async () => {
    ...
    try {
      const generatedCaption = await generateGeminiCaption(videoFile, options)
  ...
  return (
    <section className="card" aria-labelledby="video-caption-title">
      <h2 id="video-caption-title">Video Caption Generator</h2>
      ...
      <div className="preview">
        {videoUrl ? (
          <div className="video-wrap">
            <video src={videoUrl} controls style={{ width: '100%', maxWidth: '800px', borderRadius: '8px' }} />
          </div>
        ) : (
          <div className="placeholder">No video uploaded — preview appears here.</div>
        )}
      </div>
